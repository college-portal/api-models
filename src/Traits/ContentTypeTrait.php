...
trait ContentTypeTrait
{
    ...
    public static function format($data)
    {
        ...
        if (self::isArray($transformed)) {
            return self::ARRAY;
        } elseif (self::isDateTime($transformed)) {
            return self::DATETIME;
        } elseif (self::isNumber($transformed)) {
            return self::NUMBER;
        } elseif (self::isString($transformed)) {
            return self::STRING;
        } elseif (self::isBoolean($transformed)) {
            return self::BOOLEAN;
        } elseif (self::isObject($transformed)) {
            return self::OBJECT;
        } else {
            throw new Exception("type of $transformed not found");
        }
    }
