    ...
    CanResetPasswordContract
{
    ...
    {
        if ($this->hasRole(Role::ADMIN)) {
            return ChargeableService::query();
        } else {
            $ids = $this->schools()->pluck('schools.id');
    ...
    public function hasRole($role_names, $school_id = null)
    {
        ...
        if (!is_array($role_names)) {
            return $q->where('name', $role_names)->exists();
        } else {
            return $q->whereIn('name', $role_names)->exists();
        }
    }
